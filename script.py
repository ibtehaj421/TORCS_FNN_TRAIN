...
def load_and_preprocess_data(file_path):
    ...
    data['client_focus'] = parse_multi_value_column(data['client_focus'])
    
    # Input and output columns
    input_cols = ['angle', 'curLapTime', 'damage', 'distFromStart', 'distRaced', 'fuel', 'gear', 
                  'lastLapTime', 'rpm', 'speedX', 'speedY', 'speedZ', 'trackPos', 'z']
    output_cols = ['client_accel', 'client_brake', 'client_gear', 'client_steer', 'client_clutch']
...
def test_model(model, X_val, y_val, scaler_y, device, num_samples=5):
    ...
    with torch.no_grad():
        ...
        print("\nSample Predictions (accel, brake, gear, steer, clutch):")
...
def main():
    ...
    dataset_path = 'parsed_log.csv.xls'
    ...
    # Train the model
    train_model(model, X_train, y_train, X_val, y_val, device, num_epochs=100, batch_size=32, learning_rate=0.001)
    # Test the model on a few validation samples
    test_model(model, X_val, y_val, scaler_y, device, num_samples=5)
    # Save the model and scalers
    torch.save(model.state_dict(), 'torcs_controller.pth')
